...
export const POST: APIRoute = async ({ params, locals, request, redirect }) => {
  ...
  const base = `/ensembles/${id}/arrangements/${arrangementId}`;
  if (result.error) {
    return redirect(`${base}?uploadError=${encodeURIComponent(result.error)}`);
  }
